...
int test_node_viewer(const bool debug_print,
                     std::shared_ptr<ocg::Cache> cache) {
    if (debug_print) {
        std::cout << "======================= test_node_viewer()" << '\n';
    }
    ...
    // Viewer Nodess
    graph.set_node_attr_i32(viewer1_node, "bake_option_num", 1);
    ...
    graph.set_node_attr_str(
        ...
        "./tests/data/out/test_node_viewer_out1.####.jpg");
